...
class SafetyConstraintEngine:
    ...
    def validate_request(self, request: FarmerRequest) -> List[ConstraintViolation]:
        ...
        if request.crop:
            violations.extend(self._check_season_compatibility(request.crop, request.geo.region_code))
    ...
    def _check_soil_crop_compatibility(
        self,
        soil: Any,
        crop: Any
    ) -> List[ConstraintViolation]:
    ...
    def _check_action_safety(
        ...
        if any(keyword in action_lower for keyword in dangerous_keywords):
            ...
            if not any("safety" in w.lower() or "protective" in w.lower() 
