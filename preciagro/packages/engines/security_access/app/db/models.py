...
from sqlalchemy import (
    ...
    UniqueConstraint, CheckConstraint, LargeBinary
)
...
class Actor(Base):
    ...
    actor_id: Mapped[str] = mapped_column(String(100), primary_key=True, default=lambda: f"actor_{uuid4().hex[:16]}")
    ...
    service_name: Mapped[Optional[str]] = mapped_column(String(100), nullable=True, unique=True, index=True)
    service_endpoint: Mapped[Optional[str]] = mapped_column(String(500), nullable=True)
    
    # Common fields
    trust_level: Mapped[TrustLevel] = mapped_column(String(20), default=TrustLevel.UNVERIFIED, nullable=False)
    region: Mapped[Optional[str]] = mapped_column(String(100), nullable=True, index=True)
    capabilities: Mapped[dict] = mapped_column(JSONType, default=dict, nullable=False)  # JSON capabilities
    ...
    # Relationships
    tokens: Mapped[list["Token"]] = relationship(back_populates="actor", cascade="all, delete-orphan")
    roles_assigned: Mapped[list["ActorRole"]] = relationship(back_populates="actor", cascade="all, delete-orphan")
    mfa_devices: Mapped[list["MFADevice"]] = relationship(back_populates="actor", cascade="all, delete-orphan")
    audit_logs: Mapped[list["AuditLog"]] = relationship(back_populates="actor")
    
    __table_args__ = (
        Index("idx_actor_type_active", "actor_type", "is_active"),
        Index("idx_actor_region", "region", "is_active"),
        CheckConstraint("actor_type = 'human' OR (email IS NULL AND phone IS NULL)", name="check_human_fields"),
        CheckConstraint("actor_type = 'device' OR device_name IS NULL", name="check_device_fields"),
        CheckConstraint("actor_type = 'service' OR service_name IS NULL", name="check_service_fields"),
    )
...
class Role(Base):
    ...
    actors: Mapped[list["ActorRole"]] = relationship(back_populates="role", cascade="all, delete-orphan")
...
class Permission(Base):
    ...
    resource: Mapped[str] = mapped_column(String(100), nullable=False, index=True)  # e.g., "farm", "inventory", "recommendation"
...
class RolePermission(Base):
    ...
    role_id: Mapped[str] = mapped_column(String(100), ForeignKey("security_roles.role_id", ondelete="CASCADE"), primary_key=True)
    permission_id: Mapped[str] = mapped_column(String(100), ForeignKey("security_permissions.permission_id", ondelete="CASCADE"), primary_key=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
...
class ActorRole(Base):
    ...
    actor_id: Mapped[str] = mapped_column(String(100), ForeignKey("security_actors.actor_id", ondelete="CASCADE"), primary_key=True)
    role_id: Mapped[str] = mapped_column(String(100), ForeignKey("security_roles.role_id", ondelete="CASCADE"), primary_key=True)
    assigned_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    assigned_by: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)  # Actor ID who assigned
    expires_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True, index=True)
    revoked_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True, index=True)
    ...
    __table_args__ = (
        Index("idx_actor_role_active", "actor_id", "role_id", "revoked_at"),
    )
...
class Token(Base):
    ...
    token_id: Mapped[str] = mapped_column(String(100), primary_key=True, default=lambda: f"token_{uuid4().hex[:16]}")
    actor_id: Mapped[str] = mapped_column(String(100), ForeignKey("security_actors.actor_id", ondelete="CASCADE"), nullable=False, index=True)
    token_type: Mapped[TokenType] = mapped_column(String(20), nullable=False, index=True)
    token_hash: Mapped[str] = mapped_column(String(255), unique=True, nullable=False, index=True)  # Hashed token value
    jti: Mapped[Optional[str]] = mapped_column(String(100), unique=True, nullable=True, index=True)  # JWT ID
    ...
    scopes: Mapped[list[str]] = mapped_column(JSONType, default=list, nullable=False)  # List of scopes
    
    # Refresh token rotation
    parent_token_id: Mapped[Optional[str]] = mapped_column(String(100), ForeignKey("security_tokens.token_id", ondelete="SET NULL"), nullable=True)
    rotated_from_token_id: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)  # Previous token ID after rotation
...
class MFADevice(Base):
    ...
    device_id: Mapped[str] = mapped_column(String(100), primary_key=True, default=lambda: f"mfa_{uuid4().hex[:16]}")
    actor_id: Mapped[str] = mapped_column(String(100), ForeignKey("security_actors.actor_id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    __table_args__ = (
        Index("idx_mfa_actor_active", "actor_id", "is_active"),
    )
...
class EncryptionKey(Base):
    ...
    key_type: Mapped[str] = mapped_column(String(50), nullable=False)  # data_encryption, token_signing, etc.
    encrypted_key: Mapped[bytes] = mapped_column(LargeBinary, nullable=False)  # Key encrypted with master key
...
class AuditLog(Base):
    ...
    log_id: Mapped[str] = mapped_column(String(100), primary_key=True, default=lambda: f"audit_{uuid4().hex[:16]}")
    actor_id: Mapped[Optional[str]] = mapped_column(String(100), ForeignKey("security_actors.actor_id", ondelete="SET NULL"), nullable=True, index=True)
    ...
    # Event details
    resource_type: Mapped[Optional[str]] = mapped_column(String(100), nullable=True, index=True)  # e.g., "farm", "inventory"
    resource_id: Mapped[Optional[str]] = mapped_column(String(100), nullable=True, index=True)
    action: Mapped[Optional[str]] = mapped_column(String(50), nullable=True, index=True)  # e.g., "read", "write"
...
class ABACPolicy(Base):
    ...
    policy_id: Mapped[str] = mapped_column(String(100), primary_key=True, default=lambda: f"policy_{uuid4().hex[:16]}")
