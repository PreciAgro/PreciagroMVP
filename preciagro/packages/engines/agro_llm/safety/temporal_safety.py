...
class TemporalSafetyValidator:
    ...
    def validate(
        ...
        if request.crop and hasattr(request, 'geo'):
            violations.extend(self._check_phi_violations(
                response, request, current_date
            ))
        
        # Check crop stage compatibility
        if request.crop:
            violations.extend(self._check_crop_stage_compatibility(
                response, request
            ))
        
        # Check season compatibility
        violations.extend(self._check_season_compatibility(
            response, request, current_date
        ))
        
        # Check "too late in season"
        violations.extend(self._check_too_late_in_season(
            response, request, current_date
        ))
    ...
    def _check_phi_violations(
        ...
        for action in response.diagnosis_card.recommended_actions:
            ...
            for phi_rule in self.phi_rules:
                if (phi_rule.crop.lower() == crop_type and
                    phi_rule.chemical.lower() in action_text):
    ...
    def _check_crop_stage_compatibility(
        ...
        crop_type = request.crop.type.lower() if request.crop else None
        growth_stage = request.crop.growth_stage.lower() if (
            request.crop and request.crop.growth_stage
        ) else None
        ...
        for rule in self.crop_stage_rules:
            if (rule.crop.lower() == crop_type and
                rule.growth_stage.lower() == growth_stage):
                matching_rule = rule
                break
            ...
            if days_since_planting > 120:
                ...
                for action in response.diagnosis_card.recommended_actions:
                    time_sensitive_keywords = [
                        "planting", "sowing", "transplanting", "early stage"
                    ]
