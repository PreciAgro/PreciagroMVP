...
class SoilData(BaseModel):
    ...
    moisture: Optional[float] = Field(
        None, ge=0.0, le=100.0, description="Soil moisture percentage"
    )
...
class FarmerRequest(BaseModel):
    ...
    image_features: List[ImageFeature] = Field(
        default_factory=list, description="Image features with embeddings"
    )
    ...
    session_context: List[SessionContext] = Field(
        default_factory=list, description="Previous session context"
    )
...
class Explainability(BaseModel):
    ...
    confidence_breakdown: Optional[Dict[str, float]] = Field(
        None, description="Confidence breakdown by component"
    )
...
class AgroLLMResponse(BaseModel):
    ...
    diagnosis_card: DiagnosisCard = Field(
        ..., description="Structured diagnosis and recommendations"
    )
    explainability: Explainability = Field(
        default_factory=Explainability, description="Explainability information"
    )
