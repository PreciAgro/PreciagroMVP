...
class RuleExplainer(BaseExplainer):
    ...
    def explain(
        ...
        triggered_rules = model_output.get(
            "triggered_rules", model_output.get("rules", model_output.get("activated_rules", []))
        )
        ...
        # Find rule evidence
        rule_evidence = [ev for ev in evidence if ev.evidence_type == EvidenceType.RULE]

        # Generate explanation based on level
        if level == ExplanationLevel.FARMER:
            content = self._generate_farmer_explanation(recommendation, triggered_rules, language)
            content_type = "text"
        elif level == ExplanationLevel.EXPERT:
            content = self._generate_expert_explanation(recommendation, triggered_rules, evidence)
            content_type = "text"
        else:  # AUDITOR
    ...
    def trace_rules(self, model_output: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def _generate_farmer_explanation(
        self, recommendation: str, triggered_rules: List[Any], language: str
    ) -> str:
        ...
        if language == "en":
            if reasons:
                ...
                return f"Recommendation: {recommendation}. " f"This is based on: {reasons_str}."
            else:
                return f"Recommendation: {recommendation}."
        elif language == "sn":  # Shona
            if reasons:
                reasons_str = "; ".join(reasons[:3])
                return f"Zano: {recommendation}. " f"Izvi zvakabudiswa ne: {reasons_str}."
            else:
    ...
    def _generate_expert_explanation(
        self, recommendation: str, triggered_rules: List[Any], evidence: List[EvidenceItem]
    ) -> str:
        ...
        """
        parts = [f"**Recommendation**: {recommendation}", "", "**Rule Trace**:"]
    ...
    def _generate_auditor_explanation(
        self, model_output: Dict[str, Any], triggered_rules: List[Any]
    ) -> str:
