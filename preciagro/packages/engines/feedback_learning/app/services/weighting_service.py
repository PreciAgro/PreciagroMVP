...
class WeightingService:
    ...
    def _compute_factors(
        ...
        environmental_stability = self._compute_environmental_stability_factor(
            timing_context
        )
    ...
    def _compute_historical_accuracy_factor(
        ...
        if farmer_profile.total_feedback_count > 0:
            accuracy = (
                farmer_profile.accurate_feedback_count
                / farmer_profile.total_feedback_count
            )
            return accuracy
    ...
    async def get_weighted_for_recommendation(
        ...
        """
        return [
            w for w in self._weighted_store.values()
            if w.recommendation_id == recommendation_id
        ]
