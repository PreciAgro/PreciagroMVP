...
class AgroLLMWrapper:
    ...
    async def generate_response(
        self, request: FarmerRequest, context: Optional[Dict[str, Any]] = None
    ) -> AgroLLMResponse:
    ...
    def _build_prompt(self, request: FarmerRequest, context: Dict[str, Any]) -> str:
        ...
        if request.weather:
            prompt_parts.append(
                f"Weather: temp={request.weather.temp}°C, humidity={request.weather.humidity}%"
            )
    ...
    def _parse_output(self, raw_output: Dict[str, Any], request: FarmerRequest) -> AgroLLMResponse:
        ...
        diagnosis_card = DiagnosisCard(
            ...
            recommended_actions=[
                (
                    ...
                    else RecommendedAction(action=str(action))
                )
        ...
        explainability = Explainability(rationales=raw_output.get("rationales", []))
