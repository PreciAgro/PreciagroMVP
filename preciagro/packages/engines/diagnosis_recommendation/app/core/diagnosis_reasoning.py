...
class DiagnosisReasoningCore:
    ...
    def reason(
        ...
        hypotheses = [
            ...
            if h.belief_score >= settings.MIN_CONFIDENCE_THRESHOLD
        ]
    ...
    def _apply_symbolic_reasoning(
        ...
        for hypothesis in hypotheses:
            ...
            supporting_evidence = [
                ev for ev in evidence_graph.evidence
                if ev.id in hypothesis.evidence_ids
            ]
            
            if supporting_evidence:
                avg_evidence_strength = sum(ev.strength for ev in supporting_evidence) / len(supporting_evidence)
                hypothesis.belief_score = min(1.0, hypothesis.belief_score * (0.5 + avg_evidence_strength))
    ...
    def _apply_probabilistic_reasoning(
        ...
        for hypothesis in hypotheses:
            ...
            if supporting_evidence:
                ...
                evidence_likelihood = sum(ev.strength * ev.confidence for ev in supporting_evidence) / len(supporting_evidence)
                
                # Update belief score
                hypothesis.belief_score = min(
                    1.0,
                    hypothesis.prior_probability * (1.0 + evidence_likelihood)
                )
    ...
    def _calculate_overall_confidence(
        ...
        overall = sum(
            h.belief_score * w
            for h, w in zip(top_hypotheses, weights)
        )
