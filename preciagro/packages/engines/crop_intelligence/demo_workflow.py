...
def main():
    ...
    response = requests.get(
        f"{BASE_URL}/cie/field/state", params={"field_id": "demo_maize_zw_001"})
    print(f"Status: {response.status_code}")
    state = response.json()
    pretty_print(state)
    ...
    response = requests.get(
        f"{BASE_URL}/cie/field/actions", params={"field_id": "demo_maize_zw_001"})
    print(f"Status: {response.status_code}")
    ...
    for idx, action in enumerate(actions.get('items', []), 1):
        ...
        if action.get('window_start'):
            print(
                f"      Timing: {action['window_start']} to {action.get('window_end', 'TBD')}")
