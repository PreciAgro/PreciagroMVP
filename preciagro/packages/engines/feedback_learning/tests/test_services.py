...
from preciagro.packages.engines.feedback_learning.app.services.validation_service import ValidationService
from preciagro.packages.engines.feedback_learning.app.services.weighting_service import WeightingService
...
class TestWeightingService:
    ...
    async def test_weight_with_farmer_profile(self, service):
        ...
        weighted = await service.compute_weight(
            event, farmer_profile=farmer_profile
        )
...
class TestAuditService:
    ...
    async def test_add_steps_and_complete(self, service):
        ...
        await service.add_validation_step(
            trace.trace_id,
            is_valid=True,
            validation_details={"is_duplicate": False}
        )
