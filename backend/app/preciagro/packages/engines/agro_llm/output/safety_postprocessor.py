...
class SafetyPostProcessor:
    ...
    def validate_and_fix(
        self, response: AgroLLMResponse, request_context: Optional[Dict[str, Any]] = None
        ...
        for action in fixed_response.diagnosis_card.recommended_actions:
            action_violations, fixed_action = self._validate_and_fix_action(action, request_context)
        ...
        for violation in violations:
            if violation.severity == ViolationSeverity.BLOCKING:
                ...
                fixed_response.diagnosis_card.warnings.append(f"SAFETY ALERT: {violation.message}")
    ...
    def _validate_and_fix_action(
        self, action: RecommendedAction, request_context: Optional[Dict[str, Any]]
    ...
    def _check_illegal_chemicals(self, response: AgroLLMResponse) -> List[ConstraintViolation]:
    ...
    def _validate_doses(self, response: AgroLLMResponse) -> List[ConstraintViolation]:
    ...
    def _validate_timing(
        self, response: AgroLLMResponse, request_context: Optional[Dict[str, Any]]
    ...
    def _check_missing_warnings(self, response: AgroLLMResponse) -> List[ConstraintViolation]:
            ...
            if response.diagnosis_card.severity == "high":
                ...
                if has_chemical_action:
                    has_safety_warning = any(
                        "safety" in w.lower()
                        or "protective" in w.lower()
                        or "equipment" in w.lower()
    ...
    def _validate_region_codes(
        self, response: AgroLLMResponse, request_context: Dict[str, Any]
    ) -> List[ConstraintViolation]:
