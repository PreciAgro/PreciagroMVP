...
class CVExplainer(BaseExplainer):
    ...
    def explain(
        ...
        """
        # Find image evidence
        image_evidence = [ev for ev in evidence if ev.evidence_type == EvidenceType.IMAGE]

        # Extract prediction info
        diagnosis = model_output.get(
            "diagnosis", model_output.get("prediction", model_output.get("label", "Unknown"))
        )
        ...
        if image_evidence and self._can_generate_saliency():
            saliency_base64 = self._generate_saliency_thumbnail(image_evidence[0], model_output)
    ...
    def get_saliency_thumbnail(
        self, image_evidence: EvidenceItem, model_output: Dict[str, Any]
    ) -> Optional[str]:
    ...
    def _can_generate_saliency(self) -> bool:
        ...
        return (
            self._gradcam_generator is not None
            and self._gradcam_generator.available
            and HAS_NUMPY
            and HAS_CV2
        )
    ...
    def _generate_farmer_cv_explanation(
        self, diagnosis: str, confidence: float, language: str
    ) -> str:
    ...
    def _generate_auditor_cv_explanation(self, model_output: Dict[str, Any]) -> str:
