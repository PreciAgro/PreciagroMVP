...
class ImageAnalysisSignal(BaseModel):
    ...
    detected_labels: List[str] = Field(
        default_factory=list, description="Detected disease/pest labels"
    )
    confidence_scores: Dict[str, float] = Field(
        default_factory=dict, description="Confidence per label"
    )
...
class CropIntelligenceSignal(BaseModel):
    ...
    biological_constraints: Dict[str, Any] = Field(
        default_factory=dict, description="Biological constraints"
    )
...
class InventorySignal(BaseModel):
    ...
    available_inputs: Dict[str, Dict[str, Any]] = Field(
        default_factory=dict, description="Available inputs"
    )
    stock_levels: Dict[str, float] = Field(default_factory=dict, description="Stock levels")
    procurement_options: List[Dict[str, Any]] = Field(
        default_factory=list, description="Procurement options"
    )
    metadata: Dict[str, Any] = Field(default_factory=dict, description="Additional metadata")
...
class FarmerProfileSignal(BaseModel):
    ...
    budget_class: Optional[Literal["low", "medium", "high"]] = Field(
        None, description="Budget class"
    )
...
class DREInput(BaseModel):
    ...
    timestamp: datetime = Field(
        default_factory=lambda: datetime.utcnow(), description="Request timestamp"
    )

    # Optional signals from upstream engines
    image_analysis: Optional[List[ImageAnalysisSignal]] = Field(
        None, description="Image analysis signals"
    )
    ...
    crop_intelligence: Optional[CropIntelligenceSignal] = Field(
        None, description="Crop intelligence"
    )
    ...
    urgency: Literal["low", "medium", "high"] = Field(
        default="medium", description="Request urgency"
    )
    metadata: Dict[str, Any] = Field(default_factory=dict, description="Additional metadata")
...
class DiagnosisResult(BaseModel):
    ...
    primary_confidence: float = Field(
        ..., ge=0.0, le=1.0, description="Primary hypothesis confidence"
    )
...
class DREResponse(BaseModel):
    ...
    response_id: str = Field(
        default_factory=lambda: str(uuid4()), description="Response identifier"
    )
    ...
    overall_confidence: float = Field(..., ge=0.0, le=1.0, description="Overall confidence")
    uncertainty_metrics: Dict[str, Any] = Field(
    ...
    data_requests: List[DataRequest] = Field(
        default_factory=list, description="Requests for additional data"
    )
