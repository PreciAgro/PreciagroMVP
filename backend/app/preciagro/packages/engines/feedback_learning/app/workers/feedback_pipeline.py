...
def process_feedback_event(
    ...
    async def _process():
        try:
            ...
            # Parse contexts
            farmer_ctx = FarmerProfileContext(**farmer_profile) if farmer_profile else None
            rec_ctx = (
                RecommendationContext(**recommendation_context) if recommendation_context else None
            )
            timing_ctx = OutcomeTimingContext(**timing_context) if timing_context else None
