...
class TraceStoreBase(ABC):
    ...
    @abstractmethod
    async def search(self, filters: Dict[str, Any], limit: int = 100) -> List[ReasoningTrace]:
...
class SQLiteTraceStore(TraceStoreBase):
    ...
    async def retrieve(self, trace_id: str) -> Optional[ReasoningTrace]:
        ...
        with self._get_connection() as conn:
            cursor = conn.execute(
                "SELECT trace_data FROM reasoning_traces WHERE trace_id = ?", (trace_id,)
            )
            row = cursor.fetchone()
    ...
    async def delete(self, trace_id: str) -> bool:
        """Delete a trace."""
        with self._get_connection() as conn:
            cursor = conn.execute("DELETE FROM reasoning_traces WHERE trace_id = ?", (trace_id,))
            conn.commit()
    ...
    async def update_signature(self, trace_id: str, signature: str) -> bool:
...
class PostgresTraceStore(TraceStoreBase):
        ...
        if self._pool is None:
            try:
                ...
                self._pool = await asyncpg.create_pool(
                    self._connection_string, min_size=2, max_size=self._pool_size
                )
    ...
    async def retrieve(self, trace_id: str) -> Optional[ReasoningTrace]:
        ...
        async with pool.acquire() as conn:
            row = await conn.fetchrow(
                "SELECT trace_data FROM reasoning_traces WHERE trace_id = $1", trace_id
            )
    ...
    async def search(self, filters: Dict[str, Any], limit: int = 100) -> List[ReasoningTrace]:
        ...
        async with pool.acquire() as conn:
            ...
            return [ReasoningTrace.model_validate(row["trace_data"]) for row in rows]
    ...
    async def delete(self, trace_id: str) -> bool:
        ...
        async with pool.acquire() as conn:
            result = await conn.execute(
                "DELETE FROM reasoning_traces WHERE trace_id = $1", trace_id
            )
