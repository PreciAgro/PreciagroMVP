...
class Observation(BaseModel):
    ...
    timestamp: datetime = Field(default_factory=lambda: datetime.utcnow(), description="Observation timestamp")
    
    # Normalized data
    signal: str = Field(..., description="Normalized signal identifier (e.g., 'leaf_spot', 'yellowing')")
...
class Hypothesis(BaseModel):
    ...
    # Scoring
    belief_score: float = Field(..., ge=0.0, le=1.0, description="Belief score (posterior probability)")
    ...
    temporal_validity: Optional[Dict[str, Any]] = Field(None, description="Temporal validity constraints")
    spatial_validity: Optional[Dict[str, Any]] = Field(None, description="Spatial validity constraints")
...
class Recommendation(BaseModel):
    ...
    # Evidence linkage
    supporting_hypothesis_ids: List[str] = Field(default_factory=list, description="Supporting hypotheses")
...
class RecommendationPlan(BaseModel):
    ...
    execution_order: List[str] = Field(..., description="Recommended execution order (recommendation IDs)")
    ...
    # Validation
    is_validated: bool = Field(default=False, description="Whether plan passed constraint validation")
...
class UncertaintyMetric(BaseModel):
    ...
    component_uncertainties: Dict[str, float] = Field(default_factory=dict, description="Per-component uncertainty")
    missing_data: List[str] = Field(default_factory=list, description="Missing data sources")
    low_confidence_sources: List[str] = Field(default_factory=list, description="Low confidence sources")
...
class ReasoningTrace(BaseModel):
    ...
    model_inferences: List[Dict[str, Any]] = Field(default_factory=list, description="ML model inferences")
    
    # Explainability
    rationale: str = Field(..., description="Human-readable rationale")
    confidence_breakdown: Dict[str, float] = Field(default_factory=dict, description="Confidence breakdown")
