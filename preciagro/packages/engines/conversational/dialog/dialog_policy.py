...
class DialogPolicy:
    ...
    def determine_action(
        self, user_input: str, agrollm_response: Dict[str, Any], session_state: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if flags.get("needs_review"):
            return {"action": "human_handoff", "reason": "AgroLLM flagged for review"}
        ...
        if agrollm_response.get("missing_slots"):
            return {"action": "clarify", "slots": agrollm_response["missing_slots"]}

        # 4. Check for risky actions requiring confirmation
        if flags.get("high_risk"):
            return {"action": "confirm_risk", "details": agrollm_response.get("risk_details")}

        # Default: Proceed with response
        return {"action": "respond", "content": agrollm_response.get("content")}
