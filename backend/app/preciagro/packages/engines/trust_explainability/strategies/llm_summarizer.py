...
class LLMSummarizer(BaseExplainer):
    ...
    def explain(
        ...
        if level == ExplanationLevel.FARMER:
            ...
            content = self._generate_auditor_summary(prediction, evidence, model_output)
    ...
    def generate_one_liner(self, model_output: Dict[str, Any], language: str = "en") -> str:
        ...
        """
        prediction = model_output.get("diagnosis", model_output.get("prediction", "Unknown issue"))
        confidence = model_output.get("confidence", 0.0)
    ...
    def _load_templates(self) -> Dict[str, Dict[str, str]]:
        ...
        return {
            "en": {
                ...
                "recommendation": (
                    "We recommend: {action}. {reason}. " "This is based on {evidence_note}."
                ),
            },
            "sn": {  # Shona
                "farmer_high": (
                ...
                "farmer_medium": (
                    "Ichi chinogona kuva {diagnosis} ({confidence}% chivimbo). " "{evidence_note}"
                ),
                "farmer_low": (
                    "Hatisi nechokwadi, asi ichi chinogona kuva {diagnosis}. " "{evidence_note}"
                ),
            },
            "pl": {  # Polish
                "farmer_high": (
                ...
                "farmer_medium": (
                    "To może być {diagnosis} ({confidence}% pewności). " "{evidence_note}"
                ),
                "farmer_low": (
                    "Nie jesteśmy pewni, ale to może być {diagnosis}. " "{evidence_note}"
                ),
            },
        }

    def _summarize_evidence(self, evidence: List[EvidenceItem]) -> Dict[str, Any]:
        ...
        Returns:
            Summary dictionary
        """
        summary = {"count": len(evidence), "types": [], "key_points": [], "sources": set()}
    ...
    def _format_evidence_note(self, evidence_summary: Dict[str, Any], language: str) -> str:
    ...
    def _generate_farmer_summary(
        self, diagnosis: str, confidence: float, evidence_summary: Dict[str, Any], language: str
    ) -> str:
        ...
        return template.format(
            diagnosis=diagnosis, confidence=conf_pct, evidence_note=evidence_note
        ).strip()
    ...
    def _generate_expert_summary(
        ...
        """
        parts = [f"**Diagnosis**: {diagnosis}", f"**Confidence**: {confidence:.1%}", ""]
    ...
    def _generate_auditor_summary(
        self, diagnosis: str, evidence: List[EvidenceItem], model_output: Dict[str, Any]
    ) -> str:
