...
class TabularExplainer(BaseExplainer):
    ...
    def explain(
        ...
        feature_importance = model_output.get(
            "feature_importance",
            model_output.get("shap_values",
            self._compute_feature_importance(model_output))
        )
    ...
    def supports(self, model_type: str) -> bool:
        ...
        return model_type.lower() in [
            "tabular", "structured", "ml", "xgboost", "lightgbm",
            "random_forest", "gradient_boosting", "sklearn"
        ]
    ...
    def _compute_feature_importance(
        ...
        feature_keys = [
            "features", "inputs", "data", "values",
            "soil", "weather", "sensor_data"
        ]
