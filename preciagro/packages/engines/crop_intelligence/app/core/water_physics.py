...
def calculate_et0_fao56(
    ...
    rn = 0.77 * radiation_mjm2 - 0.0864 * (
        4.903e-9 * ((tmax_c + 273.16)**4 + (tmin_c + 273.16)**4) / 2.0 *
    ...
    numerator = 0.408 * delta * rn + gamma * \
        (900.0 / (tmean + 273.0)) * wind_ms * (es - ea)
...
def get_crop_kc(
    ...
    if days_since_planting < 0:
        ...
        kc = kc_data.kc_ini + (kc_data.kc_mid - kc_data.kc_ini) * \
            (days_into_stage / stage_length)
        ...
        kc = kc_data.kc_mid - (kc_data.kc_mid - kc_data.kc_end) * \
            (days_into_stage / stage_length)
...
def run_soil_water_bucket(
    ...
    for weather in weather_history:
        ...
        confidence = et0_conf * kc_conf * \
            (1.0 - soil_bucket.uncertainty_pct / 100.0)

        results.append(WaterBalance(
            date=weather.date,
            et0_mm=round(et0, 2),
            etc_mm=round(etc, 2),
            kc=round(kc, 3),
...
def get_water_stress_level(
    ...
    if crop_stage in ["development", "mid-season"]:
        ...
        if pct_available < 30:
            return "severe", 0.95, "Irrigate within 24h - critical water deficit during sensitive stage"
...
def estimate_irrigation_need(
    water_balance: List[WaterBalance],
    ...
    recent_avg_etc = sum(
        wb.etc_mm for wb in water_balance[-7:]) / min(7, len(water_balance))
