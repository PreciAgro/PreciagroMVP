...
def test_run_job_smoke(monkeypatch):
    ...
    monkeypatch.setattr(
        "preciagro.packages.engines.data_integration.pipeline.orchestrator.upsert_normalized",
        fake_upsert,
    )
    monkeypatch.setattr(
        "preciagro.packages.engines.data_integration.pipeline.orchestrator.publish_ingest_created",
        fake_publish,
    )
