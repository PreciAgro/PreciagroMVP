...
class GeoContextClient:
    ...
    async def get_field_context(
        self, lat: float, lon: float, region_code: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    async def get_weather(self, lat: float, lon: float, days: int = 7) -> Dict[str, Any]:
        ...
        return {"lat": lat, "lon": lon, "forecast_days": days, "status": "stub"}
