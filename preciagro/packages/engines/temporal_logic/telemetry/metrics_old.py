...
class TemporalEngineMetrics:
    ...
    def event_received(self, event_type: str, source: str):
        ...
        self.collector.increment(
            "events_received_total", labels={"event_type": event_type, "source": source}
        )
    ...
    # Rule metrics
    def rule_evaluated(
        self, rule_name: str, matches: bool, confidence: float, evaluation_time: float
    ):
        """Record a rule evaluation."""
        self.collector.increment(
            "rules_evaluated_total",
            labels={"rule_name": rule_name, "matches": str(matches).lower()},
        )
        self.collector.record_histogram(
        ...
        self.collector.record_histogram(
            "rule_confidence_score", confidence, labels={"rule_name": rule_name}
        )
    ...
    def task_scheduled(self, task_type: str, channel: str, delay_seconds: int):
        ...
        self.collector.increment(
            "tasks_scheduled_total", labels={"task_type": task_type, "channel": channel}
        )
        if delay_seconds > 0:
            self.collector.record_histogram(
                "task_delay_seconds", delay_seconds, labels={"task_type": task_type}
            )
    ...
    def task_executed(
        ...
        self.collector.record_histogram(
            ...
            execution_time,
            labels={"task_type": task_type, "channel": channel},
        )
        if attempts > 1:
            self.collector.increment(
                "task_retries_total",
    ...
    def message_sent(self, channel: str, success: bool, response_time: float):
        ...
        self.collector.record_histogram(
            "message_send_duration_seconds", response_time, labels={"channel": channel}
        )

    def message_delivery_status(self, channel: str, status: str):
        """Record message delivery status."""
        self.collector.increment(
            "message_delivery_status_total",
    ...
    def set_queue_depth(self, queue_name: str, depth: int):
        """Set queue depth."""
        self.collector.set_gauge("queue_depth", depth, labels={"queue": queue_name})
...
class HealthChecker:
    ...
    def get_health_summary(self) -> Dict[str, Any]:
        ...
        healthy_checks = sum(
            1 for check in self.checks.values() if check["last_status"] == "healthy"
        )
