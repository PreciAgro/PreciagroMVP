...
class EvidenceItem(BaseModel):
    ...
    timestamp: datetime = Field(
        default_factory=datetime.utcnow, description="When evidence was collected"
    )
    ...
    content_hash: Optional[str] = Field(
        None, description="SHA-256 hash of content for verification"
    )
    ...
    confidence: float = Field(
        default=1.0, ge=0.0, le=1.0, description="Evidence quality confidence"
    )
...
class ConfidenceMetrics(BaseModel):
    ...
    overall_confidence: float = Field(
        ..., ge=0.0, le=1.0, description="Overall calibrated confidence"
    )
...
class ReasoningTrace(BaseModel):
    ...
    # Input references
    input_refs: Dict[str, str] = Field(default_factory=dict, description="References to input data")

    # Models involved
    models: List[ModelInfo] = Field(default_factory=list, description="Models involved in decision")
    ...
    # Safety checks
    safety_check: Optional[SafetyCheckResult] = Field(None, description="Safety gate result")
...
class ExplanationRequest(BaseModel):
    ...
    evidence: Optional[List[Dict[str, Any]]] = Field(None, description="Pre-collected evidence")

    # Configuration
    levels: List[ExplanationLevel] = Field(
        default=[ExplanationLevel.FARMER], description="Explanation levels to generate"
    )
    include_safety_check: bool = Field(default=True, description="Whether to run safety gate")
    include_confidence: bool = Field(
        default=True, description="Whether to compute confidence metrics"
    )

    # Language
    language: Literal["en", "sn", "pl"] = Field(default="en", description="Output language")
...
class ExplanationResponse(BaseModel):
    ...
    expert_explanation: Optional[str] = Field(None, description="Detailed expert explanation")
