...
async def get_intents(
    ...
    recorded_before: Optional[datetime] = Query(
        None, description="Filter intents recorded before this date"
    ),
...
async def parse_user_input(
    ...
    try:
        ...
        # Farming intents
            ...
            word in text_lower
            for word in ["plant", "crop", "harvest", "seed", "farming", "irrigation"]
        ):
...
async def get_intents_summary(
    ...
    try:
        ...
        stmt = select(UserIntent).where(UserIntent.recorded_at >= start_date)
