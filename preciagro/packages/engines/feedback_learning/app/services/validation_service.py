...
class ValidationService:
    ...
    async def validate(
        ...
        is_contradiction, contradiction_ids = self._detect_contradictions(
    ...
    def _detect_duplicate(
        ...
        for existing in existing_events:
            ...
            if (
                ...
                time_diff = abs(
                    (event.created_at - existing.created_at).total_seconds()
                )
    ...
    def _detect_contradictions(
        ...
        if contradictions:
            logger.info(
                f"Contradictions detected for {event.feedback_id}: {contradictions}"
            )
    ...
    def _is_contradicting(
        ...
        if event.feedback_type == "explicit" and existing.feedback_type == "explicit":
            if event.rating and existing.rating:
                ...
                if (
                    event.feedback_category in positive
                    and existing.feedback_category in negative
                ) or (
                    event.feedback_category in negative
                    and existing.feedback_category in positive
                ):
                    return True
    ...
    async def validate_batch(
        ...
        for event in events:
            existing = [
                e for e in by_recommendation[event.recommendation_id]
