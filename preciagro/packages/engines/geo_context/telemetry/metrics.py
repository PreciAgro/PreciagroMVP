...
ACTIVE_CONNECTIONS = Gauge(
    'geo_context_db_connections_active',
    'Active database connections'
)

ACTIVE_REQUESTS = Gauge(
    'geo_context_active_requests',
    'Number of active geocontext requests'
)
...
class ResolverTimer:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        RESOLVER_DURATION.labels(
            resolver_type=self.resolver_type).observe(duration)
...
def _calculate_cache_hit_rate() -> Optional[float]:
    ...
    try:
        hits = CACHE_OPERATIONS.labels(
            operation="get", result="hit")._value.get()
        misses = CACHE_OPERATIONS.labels(
            operation="get", result="miss")._value.get()
        total = hits + misses
...
class StructuredLogger:
    ...
    def log_request_start(self, context_hash: str, request_data: Dict):
        """Log request start."""
        if self.enabled:
            print(f"[GEOCONTEXT] REQUEST_START context_hash={context_hash} "
    ...
    def log_request_end(self, context_hash: str, metrics: Dict):
        ...
        if self.enabled:
            ...
            print(f"[GEOCONTEXT] REQUEST_END context_hash={context_hash} "
