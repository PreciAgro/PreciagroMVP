...
def plan_actions(crop: str, diagnosis_label: str, context: dict, weather: dict) -> ActionPlan:
    ...
    """
    rationale = (
        f"Plan for {crop} in {context['region']} ({context['season']}), "
        f"weather tmax={weather['tmax_c']}C rain={weather['rain_mm']}mm."
    )
    tasks = [
        TaskItem(
            day_offset=0, title="Inspect & isolate", instructions="Remove heavily infected leaves."
        ),
        TaskItem(
            day_offset=1,
        ...
        TaskItem(
            day_offset=7, title="Follow-up scan", instructions="Capture new leaf photo to reassess."
        ),
    ]
