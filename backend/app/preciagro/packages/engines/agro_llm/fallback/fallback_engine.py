...
class FallbackEngine:
    ...
    def generate_fallback_response(
        self, request: FarmerRequest, error_context: Optional[Dict[str, Any]] = None
    ) -> AgroLLMResponse:
    ...
    def _generate_basic_fallback(
        self, request: FarmerRequest, error_context: Optional[Dict[str, Any]]
    ) -> AgroLLMResponse:
        ...
        if "yellow" in problem_text and "leaf" in problem_text:
            ...
            actions = [
                RecommendedAction(action="Check soil pH and nutrient levels", priority="high"),
                RecommendedAction(
                    action="Inspect for signs of disease or pests", priority="medium"
                ),
                RecommendedAction(action="Review irrigation schedule", priority="medium"),
            ]
        elif "pest" in problem_text or "insect" in problem_text:
            diagnosis = f"Pest or insect issue detected in {crop_type}."
            actions = [
                RecommendedAction(action="Identify the specific pest", priority="high"),
                RecommendedAction(
            ...
            actions = [
                RecommendedAction(action="Identify the specific disease", priority="high"),
                RecommendedAction(
                    action="Remove affected plants if disease is severe", priority="medium"
                ),
                RecommendedAction(
                    action="Consult agricultural expert for treatment options", priority="high"
                ),
            ]
        else:
            # Generic fallback
            actions = [
                RecommendedAction(
                    action="Consult with local agricultural extension service", priority="high"
                ),
                RecommendedAction(action="Review crop management practices", priority="medium"),
            ]
        ...
        diagnosis_card = DiagnosisCard(
            ...
            warnings=warnings,
            citations=[],
            metadata={"fallback_mode": self.mode.value, "error_context": error_context or {}},
        )
        ...
        flags = ResponseFlags(low_confidence=True, needs_review=True, safety_warning=True)
    ...
    def _generate_safe_default(
        ...
        diagnosis_card = DiagnosisCard(
            ...
            recommended_actions=[
                RecommendedAction(
                    action="Contact local agricultural extension service", priority="high"
                )
            ],
            warnings=[
