...
class TEESettings(BaseSettings):
    ...
    confidence_threshold_high: float = Field(
        default=0.8, ge=0.0, le=1.0, description="Threshold for high confidence classification"
    )
    confidence_threshold_medium: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Threshold for medium confidence classification"
    )
    confidence_threshold_action: float = Field(
        default=0.3, ge=0.0, le=1.0, description="Minimum confidence to recommend action"
    )

    # Safety gate settings
    safety_gate_enabled: bool = Field(default=True, description="Enable safety gate validation")
    safety_gate_strict: bool = Field(default=True, description="Strict mode blocks on warnings")

    # Explanation settings
    enable_shap: bool = Field(default=True, description="Enable SHAP-based explanations")
    enable_gradcam: bool = Field(default=True, description="Enable Grad-CAM saliency maps")
    enable_llm_summary: bool = Field(default=True, description="Enable LLM-based summaries")

    # Default explanation language
    default_language: str = Field(default="en", description="Default explanation language")
    ...
    max_trace_age_days: int = Field(default=90, description="Maximum trace retention in days")

    # Performance
    max_evidence_items: int = Field(default=50, description="Maximum evidence items per trace")
    ...
    api_port: int = Field(default=8008, description="API server port")
    ...
    # Integration URLs
    image_analysis_url: Optional[str] = Field(default=None, description="Image Analysis Engine URL")
