...
class ConfidenceEstimator:
    ...
    def estimate(
        ...
        component_confidences = {f"model_{i}": conf for i, conf in enumerate(raw_confidences)}
    ...
    def check_threshold(
        self, confidence: float, action_type: str = "recommendation"
    ) -> Tuple[bool, str]:
        ...
        if passes:
            ...
            message = (
                f"Confidence {confidence:.2f} below threshold {threshold:.2f} for {action_type}"
            )
    ...
    def _extract_confidences(self, model_outputs: List[Dict[str, Any]]) -> List[float]:
    ...
    def _estimate_epistemic_uncertainty(self, confidences: List[float]) -> float:
    ...
    def _estimate_aleatoric_uncertainty(self, model_outputs: List[Dict[str, Any]]) -> float:
        """Estimate aleatoric uncertainty from prediction softness.
    ...
    def _classify_uncertainty(self, epistemic: float, aleatoric: float) -> UncertaintyType:
