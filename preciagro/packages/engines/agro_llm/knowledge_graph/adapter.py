...
class KnowledgeGraphAdapter:
    ...
    def __init__(self, kg_endpoint: Optional[str] = None, kg_api_key: Optional[str] = None):
    ...
    async def query(self, subgraph_params: Dict[str, Any]) -> GraphResult:
            ...
            entities=[
                {
                    "id": "entity_1",
                    "type": (
                        subgraph_params.get("entity_types", ["crop"])[0]
                        if subgraph_params.get("entity_types")
                        else "crop"
                    ),
                    "properties": {
