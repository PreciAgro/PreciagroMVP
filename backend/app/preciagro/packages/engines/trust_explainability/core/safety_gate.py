...
class SafetyGate:
    ...
    def validate(
        self, recommendation: Dict[str, Any], context: Dict[str, Any]
    ) -> SafetyCheckResult:
        ...
        # 1. Compliance check
        if region:
            compliance_violations = self.check_compliance(recommendation, region)
    ...
    def check_compliance(
        self, recommendation: Dict[str, Any], region: str
    ...
    def check_safety_limits(self, recommendation: Dict[str, Any]) -> List[SafetyViolation]:
        ...
        if any(kw in action_text for kw in hazardous_keywords):
            ...
            if not warnings or not any(
                "protective" in str(w).lower() or "ppe" in str(w).lower() for w in warnings
            ):
                violations.append(
                    SafetyViolation(
    ...
    def check_inventory(
        self, recommendation: Dict[str, Any], inventory: Dict[str, Any]
    ) -> List[SafetyViolation]:
    ...
    def check_timing(
        self, recommendation: Dict[str, Any], crop_info: Dict[str, Any]
    ) -> List[SafetyViolation]:
    ...
    def _check_dosage(self, dose: Any, action_text: str) -> List[SafetyViolation]:
