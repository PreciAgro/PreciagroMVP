        ...
        for fact in facts:
            ...
            if key in fact_contents:
                self.violations.append(
                    f"Contradiction detected: {fact.content} vs {fact_contents[key].content}"
                )
    ...
    def _check_unsafe_sequences(self) -> None:
        ...
        for pattern in unsafe_patterns:
            if pattern[0] in " ".join(action_contents) and pattern[1] in " ".join(action_contents):
                self.violations.append(
                    f"Unsafe sequence detected: {pattern[0]} followed by {pattern[1]}"
                )
...
class ReasoningGraphEngine:
    ...
    def validate_output(
        self, llm_output: Dict[str, Any], request_context: Optional[Dict[str, Any]] = None
    ) -> ReasoningGraph:
