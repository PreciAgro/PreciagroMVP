...
async def send_message(session_id: str, request: MessageRequest, background_tasks: BackgroundTasks):
    ...
    if action["action"] == "human_handoff":
        ...
        reply_text = safety_ui.format_confirmation(
            action.get("details", "risky action"), "Potential safety risk"
        )
    ...
    background_tasks.add_task(
        feedback_client.emit_event,
        "message_processed",
        {"session_id": session_id, "user_id": request.user_id, "consent": request.consent},
    )
