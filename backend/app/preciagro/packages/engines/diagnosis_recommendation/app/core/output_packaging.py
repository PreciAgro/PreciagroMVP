...
class OutputPackagingLayer:
    ...
    def _package_diagnosis(self, diagnosis: Diagnosis) -> DiagnosisResult:
        ...
        return DiagnosisResult(
            diagnosis_id=diagnosis.id,
            primary_hypothesis=(
                diagnosis.primary_hypothesis.name if diagnosis.primary_hypothesis else "Unknown"
            ),
            primary_confidence=(
                diagnosis.primary_hypothesis.belief_score if diagnosis.primary_hypothesis else 0.0
            ),
    ...
    def _package_recommendations(
        self, recommendation_plan: RecommendationPlan
    ) -> RecommendationResult:
    ...
    def _package_warnings(
        self, recommendation_plan: RecommendationPlan, constraint_violations: list
    ) -> list[SafetyWarning]:
    ...
    def _package_data_requests(
        self, uncertainty_metric: UncertaintyMetric, context: Dict[str, Any]
    ) -> list[DataRequest]:
        ...
        for missing_source in uncertainty_metric.missing_data:
            ...
            requests.append(
                DataRequest(
                    ...
                    priority=(
                        "medium"
                        if missing_source in ["image_analysis", "crop_intelligence"]
                        else "low"
                    ),
                )
            )
    ...
    def _package_evidence_summary(self, reasoning_trace: ReasoningTrace) -> Dict[str, Any]:
