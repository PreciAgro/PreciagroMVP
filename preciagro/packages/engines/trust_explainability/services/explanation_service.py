...
class ExplanationService:
    ...
    async def explain(self, request: ExplanationRequest) -> ExplanationResponse:
        ...
        try:
            ...
            strategy_type = self.router.route(evidence, request.model_type)
            ...
            if request.include_confidence:
                confidence_metrics = self.confidence_estimator.estimate(
                    [request.model_outputs], [ev.confidence for ev in evidence]
                )
            ...
            if request.include_safety_check:
                safety_result = self.safety_gate.validate(request.model_outputs, request.context)
            ...
            trace = (
                self.trace_builder.create(request.request_id)
            ...
            response = ExplanationResponse(
                ...
                auditor_trace=(
                    reasoning_trace if ExplanationLevel.AUDITOR in request.levels else None
                ),
    ...
    async def explain_fast(self, request: ExplanationRequest) -> str:
        ...
        return summarizer.generate_one_liner(request.model_outputs, request.language)

    async def explain_deep(self, trace_id: str) -> Optional[ExplanationResponse]:
