...
class ExplicitFeedbackInput(BaseModel):
    ...
    feedback_category: Literal[
        "helpful", "not_helpful", "incorrect", "unclear", 
        "missing_info", "too_complex", "too_simple", "other"
    ] = Field(..., description="Category of feedback")
    comment: Optional[str] = Field(None, max_length=2000, description="Free-text comment")
    suggested_correction: Optional[str] = Field(None, max_length=1000, description="User's suggested fix")
...
class ImplicitFeedbackInput(BaseModel):
    ...
    device_type: Optional[Literal["mobile", "tablet", "desktop"]] = Field(None, description="Device type")
...
class OutcomeFeedbackInput(BaseModel):
    ...
    # Result assessment
    outcome_category: Optional[Literal[
        "success", "partial_success", "no_effect", "negative_effect", "unknown"
    ...
    health_score_change: Optional[float] = Field(None, ge=-100, le=100, description="Crop health change")
    
    # Evidence
    evidence_photo_refs: List[str] = Field(default_factory=list, description="Photo evidence references")
    evidence_sensor_refs: List[str] = Field(default_factory=list, description="Sensor data references")
